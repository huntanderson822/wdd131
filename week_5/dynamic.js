...
if (!list) {
  console.error("#movie-list not found");
}

 movies.forEach(({ title, date, description, imgSrc, imgAlt, ages, genre, stars }) => {
  ...
  article.innerHTML = `
    ...
    <div class="content">
      <h2>${title}</h2>
      <p class="meta">
        <time ${datetime ? `datetime="${datetime}"` : ""}>${date}</time> •
        ${genre} • Ages ${ages}
      </p>
      <p>${description}</p>
      <p class="stars" aria-label="Rating: ${stars.length} out of 5 stars">${stars}</p>
    </div>
  `;
